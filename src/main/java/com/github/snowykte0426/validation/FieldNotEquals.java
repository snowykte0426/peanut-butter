...
/**
 * 같은 그룹 내의 필드들이 서로 다른 값을 가져야 함을 검증하는 어노테이션입니다.
 * 
 * <p>사용 예시:</p>
 * <pre>
 * {@code
...
public @interface FieldNotEquals {
    ...
    /**
     * 검증 그룹을 지정합니다. 같은 그룹 내의 필드들끼리 값이 서로 다른지 검증합니다.
     * 기본값은 빈 문자열이며, 이 경우 클래스 내 모든 @FieldNotEquals 어노테이션이 적용된 필드가 하나의 그룹으로 묶입니다.
     * 
     * @return 검증 그룹 이름
     */
    ...
    /**
     * 같은 필드에 여러 개의 @FieldNotEquals 어노테이션을 적용할 때 사용됩니다.
     */
    @Target({ElementType.FIELD})
    @Retention(RetentionPolicy.RUNTIME)
    @Documented
